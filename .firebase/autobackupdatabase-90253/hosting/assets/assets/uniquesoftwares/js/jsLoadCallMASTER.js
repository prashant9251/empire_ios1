  ...
  if (MData.length > 0) {
    ...
    for (var i = 0; i < MDataLength; i++) {
      ...
      tr += `   <tr>
        ...
        <tr>
        <th colspan="2">GST : `+ getValueNotDefine(GST) + `</th>                        
        <th style="text-align:center;"><button onclick="shareTextToFlutterApp('`+encodeURIComponent(shareText)+`')">SHARE</button></th>
        </tr>
        `;
        tr += `
          <tr>
          <th colspan="3">CONTACT:-`+ (CTNM) + `<a onclick="dialNo('` + getValueNotDefine(MData[i].MO) + `')">` + getValueNotDefine(MData[i].MO) + `</a>,<a onclick="dialNo('`+ getValueNotDefine(MData[i].PH1) + `')">` + getValueNotDefine(MData[i].PH1) + `</a>,<a onclick="dialNo('` + getValueNotDefine(MData[i].PH2) + `')">` + getValueNotDefine(MData[i].PH3) + `</a></th>
          </tr>
           `;
    }
