  ...
  if (DL > 0) {
    ...
    for (var i = 0; i < DL; i++) {
      ...
      for (let m = 0; m < ccdList.length; m++) {
        ...
        <th colspan="18" class="trPartyHead2" onclick="openSubR('`+ Data[i].COD + `','` + ccdList[m] + `')">` + ccdList[m] + `</th> 
        </tr>
        ...
        for (var n = 0; n < supBillDetails.length; n++) {
          ...
          var ID = supBillDetails[n].CNO + supBillDetails[n].TYPE + supBillDetails[n].VNO;

          trRow += ` 
                 <tr class="hideAbleTr"align="center"style="">
                <td  class="selectBoxReport" style="display:none;">
                      <input type="checkbox" checked id="selectField_`+ ID + `" CNO="` + supBillDetails[n].CNO + `"DTYPE="` + supBillDetails[n].TYPE + `"VNO="` + supBillDetails[n].VNO + `"/>
                      </td>
                ...
                <td class="hideRMK" style="display:none;"onclick="openSubR('','` + Data[i].COD + `')">` + getValueNotDefine(supBillDetails[n].R) + ` </td>
                                </tr>`;
        }
        if (ccdList.length > 1) {
          trRow += `
        ...
        obj.ccode = ccode;
        obj.ccd = ccdList[m];
        obj.supBillDetails = supBillDetails;
