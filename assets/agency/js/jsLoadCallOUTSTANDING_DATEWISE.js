...
function loadCall(data) {
  ...
  DtArray = DtArray.sort(function (a, b) {
  ...
  if (DL > 0) {
    ...
    for (o = DtArray.length - 1; o >= 0; o--) {
      ...
      var Subarray = filterBy("DTE", DtArray[o]);
      ...
      if (SubarrayBLL > 0) {
        for (j = 0; j < SubarrayBLL; j++) {
          ...
          var ID = Subarray[j].CNO + Subarray[j].TYPE + Subarray[j].VNO;

          tr += ` 
          
                <tr class="hideAbleTr"align="center"style="">
                              <th  class="selectBoxReport" style="display:none;">
                              <input type="checkbox" checked id="selectField_`+ ID + `" CNO="` + Subarray[j].CNO + `"DTYPE="` + Subarray[j].TYPE + `"VNO="` + Subarray[j].VNO + `"/>
                              </th>
                              ...
                              <th class="hideRMK"style="display:none;">` + getValueNotDefine(Subarray[j].R) + ` </th>
