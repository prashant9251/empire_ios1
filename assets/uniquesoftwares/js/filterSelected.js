...
function filterSelectedInterestCalculateBill() {
            ...
            <div class="modal-body" style="width: 100%;">
                ...
                <div id="INTERESTCALCULATION" class="tabcontent">
                    <div style="display:flex;">
                        ...
                        <div style="width:50%;text-align:right;">
                            <input type="date" class="form-control" style="min-width: 50%;text-align:right;" id="PaymentDate"/>
                        </div>
                    </div>
                    <div style="display:flex;">
                        <div style="width:50%;"><b>INTEREST RATE(Yearly) % :</b></div>
                        <div style="width:50%;text-align:right;">
                            <input type="text" class="form-control" style="min-width: 50%;text-align:right;" onkeyup="localStorageSave('interestRate');" id="interestRate"/>
                        </div>
                    </div>
                    <div style="display:flex;">
                        <div style="width:50%;"><b>GRACE DAYS :</b></div>
                        <div style="width:50%;text-align:right;">
                            <input type="text" class="form-control" style="min-width: 50%;text-align:right;" onkeyup="localStorageSave('GraceDays');" id="GraceDays"/>
                        </div>
                    </div>
                    <div style="display:flex;">
                        <div style="width:50%;"><b>DAYS IN YEAR :</b></div>
                        <div style="width:50%;text-align:right;">
                            <input type="text" class="form-control"value="365" style="min-width: 50%;text-align:right;" onkeyup="localStorageSave('DaysInYear');" id="DaysInYear"/>
                        </div>
                    </div>
                </div>
                <div style="display:flex;">
                    <div style="width:50%;"></div>
                    <div style="width:50%;text-align:right;">
                        <button class="btn" onclick="callInterestCalculation();"> GENERATE REPORT</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    var GraceDays = localStorage.getItem('GraceDays');
    if (GraceDays != null) {
        $('#GraceDays').val(GraceDays);
    }
    var DaysInYear = localStorage.getItem('DaysInYear');
    if (DaysInYear != null) {
        $('#DaysInYear').val(DaysInYear);
    }

    document.getElementById("InterestDateShow").style.display = "block";
    document.getElementById("INTERESTCALCULATION").style.display = "block";

}
function callInterestCalculation() {
    document.getElementById("InterestDateShow").style.display = "none";
    interestRate = $('#interestRate').val();
    graceDays = $('#GraceDays').val();
    daysInYear = $('#DaysInYear').val();
    PaymentDate = $('#PaymentDate').val();
    boolInterestCalculate = true;
    filterSelected();
}
function closeInterestModelShowen() {
    document.getElementById("InterestDateShow").style.display = "none";
}
