...
IFSC:- *` + getValueNotDefine(DataCompmst[i].ADDRESS4) + `* \n`;
    ...
    `;
        ...
        if (reporturl.indexOf('OUTSTANDING') > -1) {
            ...
            OS_RMK = `<div id="upiQrCode"></div>
            <h4 >REMARK : </h4><h5><b>` + getValueNotDefine(OS_RMK) + `</b></h5>
...
function upiLink() {
    var upiUrl = "";
    var upi = getUrlParams(url, "upi");
    if (upi != null && upi != "" && upi != undefined && upi != "null") {
        try {
            let qrDiv = document.getElementById("upiQrCode");
