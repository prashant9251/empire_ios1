...
function createOUTSTANDINGSummeryReport() {
  ...
  <th class="hidePWSMATYPE_NAME">
  ACCOUNT&nbsp;TYPE
  <select id="SUMMERY_VIEW_TYPE" onchange="getOUTSTANDINGSummery();">
  <option>PARTY WISE</option>
  <option>BROKER WISE PARTY</option>
  </select>
  </th>
  ...
  <th class="hidePWSMPENDAMT trPartyHead">PEND.AMT <select id="PENDAMTOrder" onchange="getOUTSTANDINGSummery();"><option></option><option>ASC</option><option>DSC</option></select>
  <select id="PendAmtFilter" onchange="getOUTSTANDINGSummery();" style="width:100px;" ><option>ALL</option><option>Grater than 0</option><option>Less than 0</option></select></th>
  </th>
  </tr>
  </thead>
...
function getOUTSTANDINGSummery() {
  ...
  var PendAmtFilter = $('#PendAmtFilter').val();
  var ARR = smArray;
  ...
  if (PendAmtFilter != null && PendAmtFilter != "") {
    if (PendAmtFilter == "Grater than 0") {
      ARR = ARR.filter(function (d) {
        return d.totalAmount > 0;
      });
    } else if (PendAmtFilter == "Less than 0") {
      ARR = ARR.filter(function (d) {
        return d.totalAmount < 0;
      });
    } else if (PendAmtFilter == "ALL") {
      ARR = ARR;
    }
  }

  var smTr = '';
  var SUMMERY_VIEW_TYPE = $('#SUMMERY_VIEW_TYPE').val();
  if (SUMMERY_VIEW_TYPE == "BROKER WISE PARTY") {
    smTr = brokerWisePartyWiseSummeryTr(ARR, smTr);
  } else {
    smTr = summeryTr(ARR, smTr);
  }
...
function brokerWisePartyWiseSummeryTr(ARR, smTr) {
  var brokerList = [];
  var brokerFlg = [];
  for (let id = 0; id < ARR.length; id++) {
    var element = ARR[id];
    var broker = element.bcode;
    if (!brokerFlg[broker]) {
  ...
  $('input[type=checkbox]').each(function () {
    ...
    if (id && id.indexOf("selectFieldCode_") > -1) {
      if ($(this).is(':checked')) {
        ...
        if (ccode.length > 0) {
          var bccode = getPartyDetailsBySendCode(ccode[0].broker);
          obj.bccode = bccode.length > 0 ? bccode[0] : {};;
        }
        obj.id = id;
        obj.code = code;
